...
SELECTING_ACTION, ADDING_MEMBER, ADDING_SELF, DESCRIBING_SELF = map(
    chr, range(4)
)
...
def start(update: Update, context: CallbackContext) -> None:
    ...
    buttons = [
        [
            ...
            InlineKeyboardButton(
                text="Add yourself", callback_data=str(ADDING_SELF)
            ),
        ],
        [
    ...
    if context.user_data.get(START_OVER):
        ...
        update.message.reply_text(
            "Hi, I'm FamiliyBot and here to help you gather information"
            "about your family."
        )
...
def select_level(update: Update, context: CallbackContext) -> None:
    ...
    buttons = [
        [
            InlineKeyboardButton(
                text="Add parent", callback_data=str(PARENTS)
            ),
            InlineKeyboardButton(
                text="Add child", callback_data=str(CHILDREN)
            ),
        ],
        [
            InlineKeyboardButton(text="Show data", callback_data=str(SHOWING)),
...
def select_gender(update: Update, context: CallbackContext) -> None:
    ...
    buttons = [
        [
            ...
            InlineKeyboardButton(
                text="Add " + female, callback_data=str(FEMALE)
            ),
        ],
        [
            InlineKeyboardButton(text="Show data", callback_data=str(SHOWING)),
...
def save_input(update: Update, context: CallbackContext) -> None:
    ...
    user_data = context.user_data
    user_data[c][user_data[CURRENT_FEATURE]] = update.message.text
...
def main():
    ...
    add_member_conv = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(
                select_level, pattern="^" + str(ADDING_MEMBER) + "$"
            )
        ],
        states={
            SELECTING_LEVEL: [
    ...
    selection_handlers = [
        ...
        CallbackQueryHandler(show_data, pattern="^" + str(SHOWING) + "$"),
        CallbackQueryHandler(
            adding_self, pattern="^" + str(ADDING_SELF) + "$"
        ),
